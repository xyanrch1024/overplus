...
if ($Action -eq "configure") {
    ...
    try {
        ...
        if ($_.Exception.Message -match "MSFT_NetRouteEntryAlreadyExists") {
            Write-Host "[!] 默认路由已存在" -ForegroundColor Yellow
        } else {
            Write-Host "[!] 添加默认路由失败: $_" -ForegroundColor Yellow
        }
    }
    ...
    try {
        ...
        if ($_.Exception.Message -match "MSFT_NetRouteEntryAlreadyExists") {
            Write-Host "[!] 绕过路由已存在" -ForegroundColor Yellow
        } else {
            Write-Host "[!] 添加绕过路由失败: $_" -ForegroundColor Yellow
        }
    }

    try {
        Set-DnsClientServerAddress -InterfaceAlias $TunNic -ServerAddresses $Dns -ErrorAction Stop
        Write-Host "[+] DNS 已设为 $Dns" -ForegroundColor Green
    } catch {
        Write-Host "[!] 设置 DNS 失败: $_" -ForegroundColor Yellow
    }
...
if ($Action -eq "cleanup") {
    ...
    try {
        ...
        if ($_.Exception.Message -match "route was not found") {
            Write-Host "[!] 默认路由不存在" -ForegroundColor Yellow
        } else {
            Write-Host "[!] 移除失败: $_" -ForegroundColor Yellow
        }
    }
    ...
    try {
        ...
        if ($_.Exception.Message -match "route was not found") {
            Write-Host "[!] 绕过路由不存在" -ForegroundColor Yellow
        } else {
            Write-Host "[!] 移除失败: $_" -ForegroundColor Yellow
        }
    }
    ...
    try {
        ...
        Write-Host "[+] DNS 已恢复 DHCP" -ForegroundColor Green
    } catch {
        Write-Host "[!] DNS 恢复失败: $_" -ForegroundColor Yellow
    }
